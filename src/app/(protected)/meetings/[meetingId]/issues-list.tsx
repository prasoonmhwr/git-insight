    ...
    <div className='p-8'>
        <div className='mx-auto flex max-w-2xl items-center justify-between gap-x-8 border-b pb-6 lg:mx-0 lg:max-w-none'>
            <div className='flex items-center gap-x-6'>
                <div className="rounded-full border bg-zinc-700 p-3">
                ...
                <h1>
                    ...
                    <div className='mt-1 text-base font-semibold leading-6 text-slate-200'>
                    {meeting.name}
                    </div>
        ...
        <div className='grid grid-cols-1 gap-2 sm:grid-cols-3 '>
            {meeting.issues.map(issue => <IssueCard key={issue.id} issue={issue}/>)}
        </div>
    </div>
   </>
  )
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <blockquote className='mt-2 border-l-4 border-slate-300 bg-slate-900 p-4'>
                        <span className='text-sm text-slate-400'>
                            {issue.start} - {issue.end}
                        </span>
                        <p className='font-medium italic leading-relaxed text-slate-300'>
                            {issue.summary}
                        </p>
                    </blockquote>
                </DialogHeader>
